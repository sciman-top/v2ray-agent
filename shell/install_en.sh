...
normalizeXrayInstallSelection() {
    ...
    if [[ "${has_legacy}" == true && "${has_zero}" == false ]]; then
...
readConfigHostPathUUID() {
    ...
    if [[ -n "${configPath}" && -n "${frontingType}" ]]; then
        ...
        # Try to read alpn h2 Path
            ...
            if [[ -z "${currentPath}" && -f "${configPath}14_VLESS_XHTTP_TLS_inbounds.json" ]]; then
...
updateV2RayAgent() {
    ...
    if [[ "${release}" == "alpine" ]]; then
        wget -c -q -O /etc/v2ray-agent/install.sh --no-check-certificate "https://raw.githubusercontent.com/sciman-top/v2ray-agent/sciman-v2ray-agent/shell/install_en.sh"
    else
        wget -c -q "${wgetShowProgressStatus}" -O /etc/v2ray-agent/install.sh --no-check-certificate "https://raw.githubusercontent.com/sciman-top/v2ray-agent/sciman-v2ray-agent/shell/install_en.sh"
    fi
    ...
    echoContent skyBlue "wget -O /root/install.sh --no-check-certificate https://raw.githubusercontent.com/sciman-top/v2ray-agent/sciman-v2ray-agent/shell/install_en.sh && chmod 700 /root/install.sh && /root/install.sh"
    echo
    exit 0
}
...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "Author: sciman"; then
...
initXrayXHTTPTLSPort() {
    ...
    if [[ -z "${xHTTPTLSPort}" ]]; then
        local attempts=0
        local randomPortFound=false
        while ((attempts < 20)); do
            xHTTPTLSPort=$((RANDOM % 20001 + 10000))
            if isValidXHTTPTLSPort "${xHTTPTLSPort}" && checkPort "${xHTTPTLSPort}" transaction; then
                randomPortFound=true
                break
            fi
            attempts=$((attempts + 1))
            xHTTPTLSPort=
